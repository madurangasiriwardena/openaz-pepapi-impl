...
//public class PepRequestFactoryImpl extends PepRequestFactory {
   ...
   /**
    ...
    /**
     * Create a query PepRequest using subject, resource, action and
     * environment objects, where the resource and action objects
     * effectively define the "scope" of resources and actions
     * covered by the query. The resource and action objects generally
     * represent or are associated with a PDP policy-specific resource
     * model representation.
     * <p>
     * The results may be further "scoped" in terms of quantity, by
     * an enum that allows filtering on either allowed or denied
     * results, or all results.
     * <p>
     * In addition, the results may further controlled by a boolean
     * indicating whether brier resource-action identifier-only
     * results are returned, of full results including obligations,
     * advice, and any other details available.
     * 
     * When the PepRequest.decide() method is invoked, based on the 
     * {@link PepRespnseType} parameter, it will return one of the
     * following lists:
     * <pre>
     * 		- a list of Allowed ResourceAction pairs within scope
     * 		- a list of Denied ResourceAction pairs within scope
     ...
     * </pre>
     * and based on the resourceActionOnly boolean, the members of
     * the list will be either of:
     * <pre>
     ...
     * @param actionObj
     * @param environmentObj
     * @param responseType an enum indicating whether only-allowed results,
     *   of only denied results, or all results will be returned
     * @param resourceActionOnly a boolean indicating whether brief or
     *   verbose results will be returned
    ...
    public PepRequest newQueryPepRequest(
        ...
        PepRequest request = (PepRequest) this.newPEPRequest(
        		resourceActionOnly?
        			PepRequestOperation.QUERY: // true => brief
        			PepRequestOperation.QUERY_VERBOSE);
	...
	/**
	 * For now this is just a dummy paas thru:
	 * TBD: transform resourceActionObject to resource and action
	 * objects
	 */
	public PepRequest newQueryPepRequest(
			Object subjectObj, 
			Object resourceActionObj,
			Object environmentObj,
			PepResponseType pepResponseType,
			boolean resourceActionOnly) 
	throws PepException {
		return newQueryPepRequest(subjectObj,
				resourceActionObj, resourceActionObj,environmentObj,
				pepResponseType, resourceActionOnly);
	}
    ...
    protected PepRequest newPEPRequest(PepRequestOperation operation) {
        ...
        PepRequest pepRequest = 
        	(PepRequest)new PepRequestImpl(azReqCtx,this,operation);    
